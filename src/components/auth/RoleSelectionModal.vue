  ...
  <div v-if="show" class="role-modal-overlay">
    <div class="role-modal animate__animated animate__fadeIn">
      ...
      <div class="role-options">
        <button
          v-for="item in usuarioRolesLocales"
          :key="item.id"
          class="btn btn-role btn-pro"
          @click="$emit('select-role', { rol: item.rol.nombre, local: item.local })"
        >
          <div>{{ item.local.nombreFantasia || item.local.nombre }} ({{ item.rol.nombre }})</div>
        </button>
        <div
          v-if="!usuarioRolesLocales || usuarioRolesLocales.length === 0"
