  ...
  <div class="page page--login" :class="currentView">
    ...
    <main>
      ...
      <div
        ...
        "
        class="page-content page-content--form-steps animate__animated animate__fadeInUp"
      >
      ...
      <UserExistsModal
        v-if="showUserExistsModal"
        @close="showUserExistsModal = false"
...
export default {
  ...
  data() {
    return {
      ...
      // Tipos de local
      tipos: ['Restaurant', 'Café', 'Bar', 'Comida Rápida', 'Pizzería', 'Sushi', 'Otro'],
  ...
  methods: {
    ...
    validateRegisterForm() {
      const formValidation = useFormValidation()
      const { errors, isValid } = formValidation.validateRegisterForm(
        this.registerForm,
    ...
    validateLocalForm() {
      const formValidation = useFormValidation()
      const { errors, isValid } = formValidation.validateLocalForm(
        this.localForm,
        this.nombreEmpresa
      )
    ...
    async performLogin() {
      ...
      try {
        ...
        this.loginError = true
        this.loginMessage =
          error.response?.data?.mensaje || 'Error al iniciar sesión. Verifique sus credenciales.'
      } finally {
        // Espera mínimo 2 segundos antes de quitar el loading
        const elapsed = Date.now() - start
        if (elapsed < 2000) await new Promise((res) => setTimeout(res, 2000 - elapsed))
    ...
    async completarRegistro() {
      ...
      try {
        ...
        */
        ...
        this.registerError = true
        this.registerMessage =
          error.response?.data?.mensaje || 'Error al crear el local. Intente nuevamente.'
