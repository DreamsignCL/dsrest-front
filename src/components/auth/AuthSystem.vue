...
export default {
  ...
  data() {
    return {
      ...
      // Tipos de local (cargados dinámicamente desde la API)
      tipos: [],
  ...
  methods: {
    async cargarTiposNegocio() {
      try {
        const res = await apiService.get('tipos-negocio')
        if (Array.isArray(res)) {
          this.tipos = res.map(t => t.nombre)
        }
      } catch (error) {
        ...
        try {
          ...
          setTimeout(() => {
            if (this.selectedRole === 'Garzon') {
              ...
              this.$router.push('/garzon'); // <-- CAMBIADO A GARZON PARA PRUEBAS, ANTES ERA /admin
            }
          }, 1000);
