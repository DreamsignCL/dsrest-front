        ...
        <div class="modal-content animate__animated animate__fadeInUp modal-pago">
            ...
            <div class="modal-body">
                <div class="resumen-pago mb-3">
                    ...
                    <div class="resumen-pago__item resumen-pago__item--subtotal">
                        <span>Total afecto</span>
                        <strong>{{ formatear(totalAfecto) }}</strong>
                    </div>
                    <div class="resumen-pago__item">
                        <span>Propina</span>
                ...
                <div class="mb-3">
                    ...
                    <div class="tip-options">
                        ...
                        <button type="button" class="tip-option" :class="{ active: modoPropina === '10' }" @click="seleccionarModo('10')">10%</button>
                        <button type="button" class="tip-option" :class="{ active: modoPropina === '15' }" @click="seleccionarModo('15')">15%</button>
                        <button type="button" class="tip-option" :class="{ active: modoPropina === '20' }" @click="seleccionarModo('20')">20%</button>
                        <button type="button" class="tip-option" :class="{ active: modoPropina === 'free' }" @click="seleccionarModo('free')">Monto libre</button>
                    </div>
                    <div v-if="modoPropina === 'free'" class="input-group mt-2">
                        <span class="input-group-text">$</span>
                        <input type="number" class="form-control" min="0" v-model.number="propinaLibre" placeholder="Ingrese monto" />
                    </div>
                </div>
...
const tasaIva = 19
const totalNeto = computed(() => Math.round(subtotal.value / (1 + tasaIva / 100)))
const iva = computed(() => subtotal.value - totalNeto.value)
const totalAfecto = computed(() => subtotal.value)

const propinaPagada = computed(() => {
    if (modoPropina.value === 'none') return 0
    if (modoPropina.value === '10') return Math.round(totalAfecto.value * 0.10)
    if (modoPropina.value === '15') return Math.round(totalAfecto.value * 0.15)
    if (modoPropina.value === '20') return Math.round(totalAfecto.value * 0.20)
...
const totalAPagar = computed(() => totalAfecto.value + propinaPagada.value)
