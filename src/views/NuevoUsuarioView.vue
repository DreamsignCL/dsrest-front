...
const inicializarDatosSesion = () => {
  ...
  if (userStr && localId) {
    ...
    if (Array.isArray(userObj.localUsuarioCategorias)) {
      const relacion = userObj.localUsuarioCategorias.find((rel) => rel.localId === localId)
      if (relacion) {
  ...
  if (!formData.value.localId || !formData.value.categoriaId) {
    errorMessage.value =
      'No se pudo obtener la información del local o la categoría de la sesión. Asegúrese de que hay un local seleccionado y que el usuario tiene acceso.'
...
const triggerCreateConfirmation = () => {
  ...
  if (!formData.value.localId || !formData.value.categoriaId) {
    errorMessage.value =
      'No se pudo determinar el local o la categoría para asignar el usuario. Recargue la página o revise la sesión del administrador.'
    return
  }
...
const guardarUsuario = async () => {
  ...
  try {
    ...
    errorMessage.value =
      mensaje || 'Error al crear o asignar el usuario. Verifique los datos e intente nuevamente.'
  }
}
    ...
    >
      <div class="modal-content animate__animated animate__fadeInUp" @click.stop>
        ...
        <div class="modal-body">
          <p>
            ¿Estás seguro que deseas <strong>crear</strong> el garzón
            <strong>{{ formData.nombre }}</strong
            >?
          </p>
        </div>
        <div class="modal-footer justify-content-center">
          <button class="btn btn-outline-primary rounded-pill" type="button" @click="cancelCreate">
            Cancelar
          </button>
