                        ...
                        <div class="form-container__body pt-3">
                            ...
                            <div class="element-list pt-3">
                                <div v-for="(plato, index) in platosSeleccionados" :key="plato.menu_item_id" class="element-item element-item--add-dishes">
                                    ...
                                    <!-- Botón eliminar (solo en estado inicial) -->
                                    <button v-if="esEstadoInicial" class="btn btn-sm btn-danger rounded-circle ms-2" type="button" @click="eliminarPlato(index)">&times;</button>
                                </div>
                            </div>
                        </div>
...
const estadoActualPedido = ref(null) // { id, nombre, orden }
...
// El pedido está en estado inicial (orden=1) — se permiten quitar platos
const esEstadoInicial = computed(() => {
    if (!estadoActualPedido.value) return true // default: permitir hasta saber
    return estadoActualPedido.value.orden === 1
})
...
const eliminarPlato = (index) => {
    if (platosSeleccionados.value.length <= 1) {
        mostrarConfirmacion('El pedido debe tener al menos un plato')
        return
    }
    if (estadoActualPedido.value && estadoActualPedido.value.orden !== 1) {
        mostrarConfirmacion('Solo puedes quitar platos cuando el pedido está en el estado inicial')
        return
    }
    platosSeleccionados.value.splice(index, 1)
}
...
const abrirModalPlatos = () => {
    if (proximaTransicion.value === null) {
        mostrarConfirmacion('El pedido está en un estado final y no puede ser modificado')
        return
    }
...
const platosFiltrados = computed(() => {
    // Solo mostrar platos activos (estado = true) en el modal
    let lista = filtrarPlatosPorCategoria(filtroCategoria.value).filter(p => p.estado === true)
...
onMounted(async () => {
    ...
    if (id) {
        apiService.get(`pedidos/${id}`).then(res => {
            // Guardar estado actual del pedido
            if (res.estadoPedidoEmpresa) {
                estadoActualPedido.value = {
                    id: res.estadoPedidoEmpresa.id,
                    nombre: res.estadoPedidoEmpresa.nombre,
                    orden: res.estadoPedidoEmpresa.orden,
                }
            }
