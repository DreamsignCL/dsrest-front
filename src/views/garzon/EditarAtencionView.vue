...
const subtotal = computed(() => {
  return Math.round(
    platosSeleccionados.value.reduce((total, plato) => {
...
const textoBotonAvance = computed(() => {
  ...
  return (
    proximaTransicion.value.accionLabel ||
    proximaTransicion.value.descripcion ||
    `Marcar como ${proximaTransicion.value.nombre}`
  )
})
...
const obtenerProximaTransicion = async () => {
  try {
    ...
    proximaTransicion.value =
      Array.isArray(transiciones) && transiciones.length > 0 ? transiciones[0] : null
  } catch (error) {
...
const ejecutarAvanceEstado = async (datosPago) => {
  try {
    ...
    mostrarConfirmacion(`Pedido actualizado a ${proximaTransicion.value.nombre}`, () =>
      router.push('/garzon')
    )
  } catch (error) {
