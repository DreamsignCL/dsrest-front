      ...
      <div class="page-content page-content--backoffice-form">
        ...
        <div class="page-content__body">
          ...
          <div v-else-if="pedido" class="detalle-pedido detalle-pedido--scroll">
                    ...
                    <tr v-for="det in pedido.detalles" :key="det.id">
                      <td>
                        <div class="plato-cell">
                          <div
                            class="plato-cell__img"
                            :style="
                              det.plato?.foto ? `background-image:url(${det.plato.foto})` : ''
                            "
                          ></div>
                      ...
                      <td class="text-end">
                        ${{ Math.round(Number(det.subtotal)).toLocaleString() }}
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <!-- Resumen de la venta -->
            <section v-if="pago" class="detalle-seccion">
              ...
              <div class="resumen-venta">
                ...
                <div v-if="pago.iva != null" class="resumen-venta__row">
                  <span>IVA ({{ pago.tasaIva ?? 19 }}%)</span>
                  <span>${{ Number(pago.iva).toLocaleString() }}</span>
                </div>
                <div
                  v-if="pago.totalAfecto != null"
                  class="resumen-venta__row resumen-venta__row--subtotal"
                >
                  >
                  <span>Total afecto</span>
                  <span>${{ Number(pago.totalAfecto).toLocaleString() }}</span>
                </div>
                <div v-if="pago.propinaPagada > 0" class="resumen-venta__row">
                  <span
                ...
                <div class="resumen-venta__row resumen-venta__row--total">
                  ...
                  >${{ Number(pago.totalAPagar).toLocaleString() }}</strong
                  >
                </div>
              </div>
            </section>
...
height="16" rx="2" ry="2" />
                  ...
                  <div v-if="pago.vuelto != null">
                    ...
                    >${{ Number(pago.vuelto).toLocaleString() }}</strong
                    >
                  </div>
                </div>
              </div>
              ...
              <!-- Nota propina -->
              <div v-if="pago.propinaPagada > 0" class="propina-nota">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="15"
                  height="15"
...
y2="16" />
                ...
                Incluye propina
                <template v-if="pago.porcentajePropina"
                >del {{ pago.porcentajePropina }}%
                </template>
          ...
          <div v-else class="text-center py-5 text-muted">No se encontró el pedido.</div>
        </div>
