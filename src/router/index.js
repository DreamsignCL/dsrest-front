...
const router = createRouter({
  ...
  routes: [
    ...
    },
    {
      ...
      meta: { requiresAuth: true, allowedRoles: ['Administrador'], allowedCategories: ['Categoria 2', 'Categoria 3', 'Categoria 4'] }
    },
    {
      path: '/usuarios/nuevo',
      name: 'nuevo-usuario',
    ...
    },
    {
      ...
      meta: { requiresAuth: true, allowedRoles: ['Garzon', 'Administrador'], allowedCategories: ['Categoria 2', 'Categoria 3', 'Categoria 4'] }
    },
    {
    ...
    },
    {
      ...
      meta: { requiresAuth: true, allowedRoles: ['Administrador'], allowedCategories: ['Categoria 3', 'Categoria 4'] }
    },
    {
      path: '/categorias',
      name: 'categorias',
    ...
    },
    {
      ...
      meta: { requiresAuth: true, allowedRoles: ['Garzon'], allowedCategories: ['Categoria 2', 'Categoria 3', 'Categoria 4'] }
    },
  ],
});
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.matched.some(record => record.meta.requiresAuth)) {
    ...
    if (to.meta.allowedRoles) {
      const userRoles = user?.usuarioRolesLocales
        ?.filter(url => url.localId === sessionLocalId)
        ?.map(url => url.rol?.nombre) || []
      if (!user || !userRoles.some(rolNombre =>
        to.meta.allowedRoles.includes(rolNombre)
      )) {
    ...
    if (to.meta.allowedCategories) {
      ...
      if (user.localUsuarioCategorias && user.localUsuarioCategorias.length > 0) {
        hasCategory = user.localUsuarioCategorias.some(luc =>
    ...
    if (to.name === 'editar-usuario' && to.params.id) {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/usuarios/${to.params.id}`, {
        ...
        if (response.ok) {
          ...
          if (usuarioEditar.localUsuarioCategorias && usuarioEditar.localUsuarioCategorias.length > 0) {
            usuarioEditar.localUsuarioCategorias.forEach(luc => localesRelacionados.add(luc.localId))
          }
